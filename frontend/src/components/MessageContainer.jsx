import React from "react";
import SendInput from "./SendInput";
import Messages from "./Messages";
import { useSelector } from "react-redux";

const MessageContainer = () => {
  const { selectedUser, authUser, onlineUser } = useSelector(
    (store) => store.user,
  );
  const isOnline = onlineUser?.includes(selectedUser?._id);
  return (
    <>
      {selectedUser !== null ? (
        <div className="w-full md:min-w-[550px] md:max-w-[550px] h-[100dvh] md:h-[80vh] flex flex-col overflow-hidden">
          <div className="flex-shrink-0 flex gap-4 items-center bg-gray-800 rounded-lg p-2 pl-12 md:pl-4">
            {/* Profile photo  */}
            <div className={`avatar ${isOnline ? "avatar-online" : ""}`}>
              <div className="w-10 rounded-full">
                <img src={selectedUser?.profilePhoto} alt="userProfile" />
              </div>
            </div>
            {/* Name  */}
            <div>{selectedUser?.fullname}</div>
          </div>
          <Messages />
          <SendInput />
        </div>
      ) : (
        <div className="w-full md:min-w-[550px] md:max-w-[550px] h-[100dvh] md:h-[80vh] flex flex-col overflow-hidden">
          <h1 className="text-2xl md:text-4xl text-white font-bold">
            Hi, {authUser?.fullname}{" "}
          </h1>
          <h1 className="text-lg md:text-2xl text-white mt-2">
            Let's start conversation
          </h1>
        </div>
      )}
    </>
  );
};

export default MessageContainer;
